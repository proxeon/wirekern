...
use wirekern::{Client, Error, USER_AGENT};
...
impl Server {
    ...
    /// Test seam: inject a mocked `Client` the same way `wirekern-serve` does.
    ...
    fn initialize(&self, params: Value) -> Result<Value, Value> {
        ...
        Ok(json!({
            ...
            "instructions": "Local wirekern execution kernel. Credentials stay in the operator vault. WhatsApp sends require allow_send=true. Interactive auth stays on the CLI."
        }))
    }
...
mod tests {
    ...
    async fn initialize_echoes_supported_protocol_version() {
        ...
        assert_eq!(reply["result"]["serverInfo"]["name"], "wirekern");
    }
    ...
    async fn crlf_lines_and_null_arguments_are_accepted() {
        ...
        assert_eq!(init["result"]["serverInfo"]["name"], "wirekern");
        let listed = rpc(
            &server,
    ...
    impl wirekern::Publisher for PostMock {
        fn site(&self) -> &wirekern::Site {
            &self.site
        }
        fn capabilities(&self) -> &[wirekern::Capability] {
            &[wirekern::Capability::PublishText]
        }
        fn auth_kind(&self) -> wirekern::AuthKind {
            wirekern::AuthKind::None
        }
        async fn publish(
            &self,
            _app: &wirekern::AppConfig,
            _creds: &wirekern::AccountCreds,
            _intent: wirekern::Intent,
            _deadline: wirekern::Deadline,
    ...
    fn mock_post_server() -> Server {
        let mock = Arc::new(PostMock {
            site: wirekern::Site::new("threads"),
            posts: std::sync::atomic::AtomicUsize::new(0),
        });
        let mut registry = wirekern::Registry::new();
        registry.register(mock);
        let vault = Arc::new(wirekern::MemoryVault::new());
        vault
            .put(
                &wirekern::AccountKey::new("threads", "default"),
                &wirekern::AccountCreds::OAuth2 {
            ...
            .unwrap();
        let apps = Arc::new(wirekern::MemoryAppStore::new());
        let client = Client::new(registry, vault, apps);
        let dummy = Client::new(
            wirekern::Registry::new(),
            Arc::new(wirekern::MemoryVault::new()),
            Arc::new(wirekern::MemoryAppStore::new()),
        );
    ...
    #[async_trait::async_trait]
    impl wirekern::Publisher for WaMock {
        fn site(&self) -> &wirekern::Site {
            &self.site
        }
        fn capabilities(&self) -> &[wirekern::Capability] {
            &[wirekern::Capability::SendText]
        }
        fn auth_kind(&self) -> wirekern::AuthKind {
            wirekern::AuthKind::StaticToken
        }
        async fn publish(
            &self,
            _app: &wirekern::AppConfig,
    ...
    #[async_trait::async_trait]
    impl wirekern::WhatsAppSender for WaMock {
        async fn send_whatsapp(
            &self,
            _app: &wirekern::AppConfig,
            _creds: &wirekern::AccountCreds,
            _request: &wirekern::WhatsAppSendRequest,
            _deadline: wirekern::Deadline,
        ) -> Result<wirekern::Outcome, Error> {
    ...
    fn mock_whatsapp_server() -> Server {
        let mock = Arc::new(WaMock {
            site: wirekern::Site::new("whatsapp_cloud"),
            sends: std::sync::atomic::AtomicUsize::new(0),
        });
        let mut deny_registry = wirekern::Registry::new();
        deny_registry.register_connector(
            wirekern::Connector::from_publisher(mock.clone()).whatsapp(mock.clone()),
        );
        let mut allow_registry = wirekern::Registry::new();
        ...
        vault
            .put(
                &wirekern::AccountKey::new("whatsapp_cloud", "default"),
                &wirekern::AccountCreds::BotToken {
        ...
        let allow = Client::new(allow_registry, vault, apps)
            .with_whatsapp_policy(Arc::new(wirekern::AllowWhatsAppSendsPolicy));
