//! `wirekern post` — flags compile to Client::publish / probe.
...
use wirekern::connectors::threads::validate_text;
use wirekern::{AccountKey, Body, Client, Deadline, Error, Image, Intent, PostRequest, Site};
...
/// Reject a multi-image command shape that Wirekern cannot map to one honest
        ...
        match client.publish(&key, intent, deadline).await {
            Ok(o) => {
                ...
                if prev.as_ref().map(|s| s.is_empty()).unwrap_or(true) {
                    ...
                    code = e.exit_code();
                    results.push(serde_json::to_value(wirekern::WireError::from(&e)).unwrap());
                    break;
                }
...
pub(crate) async fn run(
    ...
    // combination names a wire contract wirekern has not verified
