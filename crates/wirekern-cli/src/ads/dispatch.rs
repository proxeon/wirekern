//! `wirekern ads` dispatch and spend-shaped policy opt-in.
...
use std::path::Path;
use std::str::FromStr;
use wirekern::{AccountKey, Client, Deadline, DraftStep, FileDraftStore, RunPausedDraft, Site};
...
pub(crate) fn apply_lifecycle_policy(client: Client, command: &AdsCmd) -> Client {
    match command {
        AdsCmd::Activate {
            allow_activate: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::Activate,
        ))),
        AdsCmd::Archive {
            allow_archive: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::Archive,
        ))),
        AdsCmd::Delete {
            allow_delete: true, ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::Delete,
        ))),
        AdsCmd::Duplicate {
            allow_duplicate: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::Duplicate,
        ))),
        AdsCmd::UpdateBudget {
        ...
        | AdsCmd::UpdateLifetimeBudget {
            ...
            wirekern::AdsAction::UpdateBudget,
        ))),
        AdsCmd::UpdateBid {
            allow_bid_edit: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::UpdateBid,
        ))),
        AdsCmd::UpdateSchedule {
            allow_schedule_edit: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::UpdateSchedule,
        ))),
        AdsCmd::UpdatePlacement {
            allow_placement_edit: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::UpdatePlacement,
        ))),
        AdsCmd::UpdateTargeting {
            allow_targeting_edit: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::UpdateTargeting,
        ))),
        AdsCmd::SwapCreative {
            allow_creative_swap: true,
            ..
        } => client.with_ads_policy(std::sync::Arc::new(wirekern::AllowAdsActionPolicy::new(
            wirekern::AdsAction::SwapCreative,
        ))),
        _ => client,
    }
}
            ...
            let request = build_link_ad_creative_request(
                ...
                LinkCreativeOptions {
                    ...
                    whatsapp_identity: whatsapp_identity_id.map(|identity_id| {
                        wirekern::WhatsAppStatusIdentity {
                            identity_id,
