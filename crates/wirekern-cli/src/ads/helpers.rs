...
pub(crate) async fn one_video_creative(
    client: &Client,
    key: &AccountKey,
    request: wirekern::CreateVideoAdCreativeRequest,
    wait: bool,
    deadline: Deadline,
    json: bool,
) -> Result<(), i32> {
    if wait {
        match client
            ...
            .await
        {
            Ok(wirekern::AdVideoWait::Ready(_)) => {}
            Ok(wirekern::AdVideoWait::Error(status)) => {
                return Err(fail(
                    &Error::InvalidQuery {
                        site: key.site.clone(),
            ...
            Ok(wirekern::AdVideoWait::Pending(status)) => {
                return Err(fail(
                    &Error::InvalidQuery {
                        site: key.site.clone(),
...
pub(crate) fn build_upload_ad_video_request(
    ...
    filename: String,
    bytes: Vec<u8>,
) -> Result<wirekern::UploadAdVideoRequest, Error> {
    let request = wirekern::UploadAdVideoRequest {
        account: ad_account,
        filename,
        bytes,
    };
...
pub(crate) async fn one_video_upload(
    client: &Client,
    key: &AccountKey,
    request: wirekern::UploadAdVideoRequest,
    wait: bool,
    deadline: Deadline,
    json: bool,
) -> Result<(), i32> {
...
pub(crate) async fn one_video_status(
    ...
    let request = wirekern::AdVideoStatusRequest { video_id: id };
    request
        .validate()
        ...
        match client.wait_for_ad_video(key, request, deadline).await {
            Ok(waited) => {
                emit_ok(&waited, json, || match &waited {
                    wirekern::AdVideoWait::Ready(status) => {
                        format!("video {} status=ready", status.video_id)
                    }
                    wirekern::AdVideoWait::Error(status) => {
                        format!("video {} status=error", status.video_id)
                    }
                    wirekern::AdVideoWait::Pending(status) => {
...
pub(crate) struct LinkCreativeOptions {
    ...
    pub(crate) whatsapp_identity: Option<wirekern::WhatsAppStatusIdentity>,
}
...
) -> Result<wirekern::CreateVideoAdCreativeRequest, Error> {
    let call_to_action = LinkCallToAction::from_str(&options.call_to_action)
        .map_err(|reason| ads_input_error(site, reason))?;
    let request = wirekern::CreateVideoAdCreativeRequest {
        account: options.ad_account,
        creative: wirekern::VideoAdCreative {
            name: options.name,
            page_id: options.page_id,
...
pub(crate) fn build_paused_adset_request(
    ...
    let bid_strategy = BidStrategy::from_str(&options.bid_strategy)
        .map_err(|reason| ads_input_error(site, reason))?;
    let billing_event = wirekern::BillingEvent::from_str(&options.billing_event)
        .map_err(|reason| ads_input_error(site, reason))?;
    let optimization_goal = wirekern::OptimizationGoal::from_str(&options.optimization_goal)
        .map_err(|reason| ads_input_error(site, reason))?;
...
pub(crate) struct PausedAdsetOptions {
    ...
    pub(crate) promoted_object: Option<wirekern::PromotedObject>,
}
...
pub(crate) fn build_promoted_object(
...
) -> Result<Option<wirekern::PromotedObject>, Error> {
    let kinds = [
        page_id.is_some(),
        pixel_id.is_some(),
    ...
    let object = if let Some(page_id) = page_id {
        wirekern::PromotedObject::Page { page_id }
        ...
        let custom_event_type = wirekern::CustomEventType::from_str(&custom_event_type)
            .map_err(|reason| ads_input_error(site, reason))?;
        wirekern::PromotedObject::Pixel {
            pixel_id,
            custom_event_type,
        }
        ...
        let custom_event_type = wirekern::CustomEventType::from_str(&custom_event_type)
            .map_err(|reason| ads_input_error(site, reason))?;
        wirekern::PromotedObject::ProductSet {
            product_set_id,
            custom_event_type,
        }
    };
...
fn build_ad_targeting(options: &PausedAdsetOptions) -> Result<wirekern::AdTargeting, String> {
    ...
    let instagram_positions = options
        ...
        .map(|p| p.parse())
        .collect::<Result<Vec<_>, _>>()?;
    Ok(wirekern::AdTargeting {
        geo_locations: wirekern::GeoLocations { countries },
...
pub(crate) fn build_insights_query(
    ...
    let report = wirekern::InsightsReportKind::from_str(&options.report).map_err(bad)?;
...
pub(crate) fn emit_insights_reply(reply: &wirekern::InsightsReply, json: bool) -> Result<(), i32> {
    if json {
        emit_raw(&serde_json::to_value(reply).expect("json"));
    } else {
        human_line(format!(
...
pub(crate) async fn run_async_insights(
    ...
    let waited = client
        ...
        .map_err(|e| fail(&e, json))?;
    match waited {
        wirekern::InsightsJobWait::Completed(reply) => emit_insights_reply(&reply, json),
        other => {
            if json {
