//! `wirekern apps` — show and write local OAuth app files.
use crate::app::{check_name, fail};
use crate::output::{emit_raw, human_line};
use clap::Subcommand;
use std::path::Path;
use wirekern::{app_source, AppConfig, AppStore, FileAppStore, OAuthApp, Site};
    ...
    match cmd {
        AppsCmd::Set {
            ...
            if app_source(&Site::new(&site)) == "env" {
                ...
                eprintln!(
                    "note: WIREKERN_{key}_CLIENT_ID/_CLIENT_SECRET are set and take precedence over apps/{site}.json"
                );
            }
            if json {
