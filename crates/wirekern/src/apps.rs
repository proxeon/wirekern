...
impl WhatsAppEnvOverride {
    fn from_process() -> Self {
        Self {
            phone_number_id: std::env::var("WIREKERN_WHATSAPP_PHONE_NUMBER_ID").ok(),
            app_secret: std::env::var("WIREKERN_WHATSAPP_APP_SECRET").ok(),
            waba_id: std::env::var("WIREKERN_WHATSAPP_WABA_ID").ok(),
            business_id: std::env::var("WIREKERN_WHATSAPP_BUSINESS_ID").ok(),
            verify_token: std::env::var("WIREKERN_WHATSAPP_VERIFY_TOKEN").ok(),
        }
    }
...
/// OAuth sites use `WIREKERN_THREADS_CLIENT_ID` / `_CLIENT_SECRET` /
...
pub fn env_override(site: &Site) -> Option<AppConfig> {
    ...
    let id = std::env::var(format!("WIREKERN_{key}_CLIENT_ID")).ok()?;
    let secret = std::env::var(format!("WIREKERN_{key}_CLIENT_SECRET")).ok()?;
    let redirect = std::env::var(format!("WIREKERN_{key}_REDIRECT_URI"))
...
mod tests {
    ...
    fn app_source_reports_env_shadowing() {
        ...
        std::env::remove_var("WIREKERN_ZZENVTESTS_CLIENT_ID");
        assert_eq!(app_source(&site), "file");

        std::env::set_var("WIREKERN_ZZENVTESTS_CLIENT_ID", "id");
        // client_id alone is not enough — env_override needs the secret too
        assert_eq!(app_source(&site), "file");

        std::env::set_var("WIREKERN_ZZENVTESTS_CLIENT_SECRET", "sec");
        assert_eq!(app_source(&site), "env");

        std::env::remove_var("WIREKERN_ZZENVTESTS_CLIENT_ID");
        std::env::remove_var("WIREKERN_ZZENVTESTS_CLIENT_SECRET");
    }
