...
impl Publisher for Threads {
    ...
    async fn probe(
        ...
        let Body::Text { text } = &intent.body else {
            ...
            // different wire form wirekern has not verified; refusing beats
        ...
        Ok(Probe {
            site: self.site.clone(),
            container_id,
            // Meta discards unpublished containers after 24h; wirekern
...
mod tests {
    ...
    async fn live_post() {
        ...
        let token = std::env::var("WIREKERN_THREADS_TOKEN").expect("WIREKERN_THREADS_TOKEN");
        let t = Threads::new().unwrap();
        ...
        let out = t
            .publish(
                &empty_app(),
                &creds,
                text_intent("wirekern live test"),
                Deadline::from_secs(30),
            )
            .await
            .unwrap();
