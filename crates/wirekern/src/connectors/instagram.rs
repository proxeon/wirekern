...
/// check shared by single-image and carousel paths guarantees Wirekern never
...
async fn list_published_media(
    ...
    // Honor the public Wirekern bound even if a remote regression ignores our
...
async fn read_json(response: reqwest::Response, site: &Site) -> Result<Value, Error> {
    ...
    // reqwest diagnostic, so retain only Wirekern's generic network message.
    let text = response
        .text()
        .await
...
mod tests {
    ...
    async fn auth_finish_exchanges_and_stores_long_lived_token_and_user_id() {
        ...
        let me = server.mock(|when, then| {
            ...
            then.status(200)
                .json_body(json!({ "user_id": "178900", "username": "wirekern_test" }));
        });
    ...
    async fn whoami_accepts_user_id_and_username() {
        ...
        assert_eq!(me_out.handle.as_deref(), Some("wirekern_test"));
    }
