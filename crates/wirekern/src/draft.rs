...
record it with `wirekern ads adopt-draft-step`. If it does not, create it with the \
...
mod tests {
    ...
    fn example_manifest_json() -> String {
        json!({
            ...
            "campaign": {
                "name": "Wirekern launch — do not activate",
                "objective": "awareness",
    ...
    fn file_store_creates_owner_only_refuses_duplicates_and_locks() {
        let dir = std::env::temp_dir().join(format!("wirekern-draft-{}", std::process::id()));
    ...
    fn state_file_is_owner_only() {
        let dir = std::env::temp_dir().join(format!("wirekern-draft-mode-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
