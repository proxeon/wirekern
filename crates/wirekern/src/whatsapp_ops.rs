...
fn constant_eq(a: &str, b: &str) -> bool {
    ...
    use sha2::Sha256;
    type H = Hmac<Sha256>;
    let mut left = <H as Mac>::new_from_slice(b"wirekern-verify-token").expect("hmac key");
    let mut right = <H as Mac>::new_from_slice(b"wirekern-verify-token").expect("hmac key");
...
/// Safe-to-display metadata for a signed webhook that Wirekern could verify
...
/// never written below the Wirekern home directory.
...
///
/// The 32-byte key comes from `WIREKERN_WHATSAPP_REPLAY_DLQ_KEY` and is never
/// generated or persisted by Wirekern. Losing it makes the captured events
...
impl EncryptedFileWhatsAppDeadLetters {
    ...
    pub fn from_env(home: impl AsRef<Path>) -> Result<Option<Self>, Error> {
        match std::env::var("WIREKERN_WHATSAPP_REPLAY_DLQ_KEY") {
    ...
    fn aad(summary: &WhatsAppDeadLetterSummary) -> Vec<u8> {
        ...
        format!(
            "wirekern-whatsapp-replay-dlq/v1|{}|{}|{}|{}",
