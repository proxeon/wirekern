...
/// narrowed by Wirekern's local validation.
...
pub enum WhatsAppMessage {
    ...
    /// a customer-service window is open; file-backed Wirekern clients reject
...
pub struct WhatsAppTemplateQuery {
    ...
    /// interpreted or reconstructed by Wirekern: Meta owns its format.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// reads. A cursor is untrusted opaque data returned by Meta, so Wirekern only
...
/// `from` and `text` are personal data, so Wirekern returns them only to the
...
pub struct InboundMessage {
    ...
    /// Wirekern does not download the bytes; the operator's webhook host can
...
/// The small, closed set of outbound delivery states Wirekern can interpret.
...
/// Wirekern does not deduplicate, reorder, persist, or infer a final state from
