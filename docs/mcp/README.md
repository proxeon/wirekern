...
`wirekern-mcp` is the local Model Context Protocol face of the same
`Client::from_home` used by the CLI and `wirekern serve`. An agent host
...
Rust SDK. Wirekern does **not** depend on `rmcp` 3.x: that crate's
...
- The host launches `wirekern mcp` (or the `wirekern-mcp` binary) as a
  subprocess with the operator's `HOME` / `WIREKERN_HOME`.
...
- Auth is the OS user plus the local vault. There is no `pk_live_` on
  ...
  operator just spawned. HTTP callers still use `wirekern serve`.
...
`WIREKERN_HOME` may be set in the host's `env` block when the vault is
not `~/.wirekern`.
...
Streamable HTTP is `wirekern serve`, not this crate.
...
Omit `--json`: stdout is the JSON-RPC pipe. `--home` / `WIREKERN_HOME`
